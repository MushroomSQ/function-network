...
class PQNET(object):
    ...
    def load_network(self, config):
        ...
        if config.module == 'graph':
            graph = get_network('graph', config)
            graph_model_path = os.path.join(config.exp_dir, "model_graph/latest.pth")
            graph.load_state_dict(torch.load(graph_model_path)['model_state_dict'])
            print("Load pre-trained graph from: {}".format(graph_model_path))
            self.graph = graph.cuda().eval()
    ...
    def set_data(self, data):
        ...
        node_vecs = self.graph(nodes, edges, nodes_to_graph)
        node_vecs = node_vecs.view(batch_size, max_n_parts, -1).transpose(0, 1)
        part_codes[1:] = node_vecs[1:]

        cond_pack = cond.unsqueeze(0).repeat(affine.size(0), 1, 1)
        self.input_seq = torch.cat([part_codes, cond_pack], dim=2)
    ...
    def reconstruct(self, data):
        ...
        self.set_data(data)
        length = self.input_seq.size(0)
        # self.input_seq = self.input_seq[0].unsqueeze(0)
        print(self.input_seq.shape)
        self.decode_seq(self.encode_seq(self.input_seq), length=length)
    ...
    def transform_points(self, points, values):
        ...
        for idx in range(len(points)):
            ...
            part_translation = self.output_affine[idx, 0, :3].reshape(1, 3) * 256
            ...
            mins = part_translation - part_size * 64 / 2
            maxs = part_translation + part_size * 64 / 2
    ...
    #              all_values: (n_points, )
    #     """
    #     mesh_extractor = MISE(self.vox_dim-1, self.upsampling_steps, self.threshold)
