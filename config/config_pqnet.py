...
class PQNetConfig(object):
    ...
    def __init__(self, phase):
        ...
        if self.exp_name is None:
            self.exp_name = "scene-generation"
        self.exp_dir = os.path.join(args.proj_dir, args.category, self.exp_name)
        ...
        # max number of parts
        #MAX_N_PARTS_DICT = {"Chair": 9, "Table": 10, "Lamp": 7,"scene":24}
        MAX_N_PARTS_DICT = {"scene": 24, 'easyscene': 4,
                            "Backpack": 2, "Basket": 4,"Bathtub": 3, "Bed": 3, "Bench":3,
                            "Bicycle": 3, "Bowl": 3, "Chair": 3, "Cup": 3, "Desk": 18 ,
                            "DryingRack": 9, "Handcart": 6, "Hanger": 3, "Hook": 4, "Lamp": 3,
                            "Laptop": 3, "Shelf": 24, "Sink": 4, "Sofa": 3, "Stand": 4,
                            "Stool": 3, "Stroller":5, "Table": 21, "Tvbench": 17, "Vase": 3}
        self.max_n_parts = MAX_N_PARTS_DICT[args.category]

        # pretrain partae path
        if self.partae_modelpath is None:
        ...
        if self.partae_modelpath is None and args.module == 'seq2seq':
            self.partae_modelpath = os.path.join(self.exp_dir, "model_part_ae/latest.pth")
        elif self.partae_modelpath is None and args.module == 'graph':
            self.partae_modelpath = os.path.join(self.exp_dir, "model_part_ae/latest.pth")
        elif self.partae_modelpath is None and args.module == 'seq2seq_loss':
            self.partae_modelpath = os.path.join(self.exp_dir, "model_part_ae/latest.pth")'''
    ...
    def _add_basic_config_(self, parser):
        ...
        group.add_argument('--module', type=str, choices=['part_ae', 'seq2seq', 'graph', 'seq2seq_loss'], required=True,
    ...
    def _add_dataset_config_(self, parser):
        ...
        group.add_argument('--data_root', type=str, default=DATA_PATH, help="file path to data")
        group.add_argument('--category', type=str, default="Chair",
                           choices=['Backpack','Basket','Bathtub','Bed','Bench',
                                    'Bicycle','Bowl','Chair','Cup','Desk',
                                    'DryingRack','Handcart','Hanger','Hook','Lamp',
                                    'Laptop','Shelf','Sink','Sofa','Stand',
                                    'Stool','Stroller','Table','Tvbench','Vase','easyscene'],
