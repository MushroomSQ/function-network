...
class Encoder3D(nn.Module):
    def __init__(self, n_layers, ef_dim=32, z_dim=128):
        ...
        self.onehot = Linear1(117, 128)
        self.linear = Linear1(256, 128)

    def forward(self, x, categories):
        out = self.model(x)
        out = out.view(out.shape[0], -1)

        cate = self.onehot(categories)
        out = torch.cat([out, cate], dim=1)
        out = self.linear(out)
        return out
...
class PartImNetAE(nn.Module):
    ...
    def forward(self, points, vox3d, categories):
        """
        ...
        z = self.encoder(vox3d, categories)  # (shape_batch_size, z_dim)
