...
# ×Ô¼º¼ÓµÄ£¬Ö»ÄÜ3¸öÎïÌå
def draw_voxel_manymodel(voxels, n_parts, is_show=True, save_path=None):
    import matplotlib.pyplot as plt
    if not is_show:
        import matplotlib
        matplotlib.use('Agg')
    ...
    colors = np.array([[92,216,241], [104,239,80], [222,151,230],[245, 201, 46]])
    colors = colors/255
    for idx in range(n_parts):
        ax.view_init(elev, azim)
        #print(idx)
        ax.voxels(voxels[idx], facecolors=colors[idx])
